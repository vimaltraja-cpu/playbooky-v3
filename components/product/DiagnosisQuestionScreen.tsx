...
type DiagnosisQuestionScreenProps = {
  currentStep?: number;
  framed?: boolean;
  initialSelectedIdsByQuestion?: Partial<Record<DiagnosisQuestionId, string[]>>;
  isLoading?: boolean;
  onBack?: () => void;
  onContinue?: () => void;
  onSelectionsChange?: (
...
export function DiagnosisQuestionScreen({
  ...
  totalSteps = 5,
  viewport = "desktop",
  initialSelectedIdsByQuestion,
  onSelectionsChange,
  progressResolvedCount = 0
  ...
  const progressStep = diagnosisProgressSteps.find(
    (step) => step.id === activeQuestion.id
  );
  ...
  const [selectedIdsByQuestion, setSelectedIdsByQuestion] = useState<
    Partial<Record<DiagnosisQuestionId, string[]>>
  >(
    initialSelectedIdsByQuestion
      ? { ...initialSelectedIdsByQuestion }
      : {
  ...
  const progressPercentage =
    activeTotalSteps > 0
      ? Math.round(
          ((progressResolvedCount + answeredQuestionCount) / activeTotalSteps) *
            100
        )
      : 0;
  ...
  const handleToggleOption = (optionId: string) => {
    setSelectedIdsByQuestion((currentSelections) => {
      ...
      let nextSelections: Partial<Record<DiagnosisQuestionId, string[]>>;
      ...
      if (currentIds.includes(optionId)) {
        ...
        onSelectionsChange?.(
          nextSelections as Partial<Record<DiagnosisQuestionId, string[]>>
        );
    ...
    >
      <div className="diagnosis-content-shell">
        ...
        <DiagnosisBottomRegion
          ...
          currentStep={progressResolvedCount + activeStepNumber}
          isLoading={isLoading}
          onBack={handleBack}
