...
const warnedMissingIllustrations = new Set<string>();
...
export function getActivitySlug(value: string) {
  return normalizeForMatch(value).replace(/\s+/g, "-");
}
...
export function getCanonicalActivityCards() {
  ...
  return parseCsv(readFileSync(csvPath, "utf8")).map((row: CsvRow) => {
    ...
    if (
      ...
      !warnedMissingIllustrations.has(title)
    ) {
      warnedMissingIllustrations.add(title);
      console.warn(
        `[ActivityCard] Missing processed illustration for "${title}". Rendering placeholder illustration.`
      );
    }
