...
type ActivityLibrarySideScrollPlaygroundProps = {
...
export function ActivityLibrarySideScrollPlayground({
  activities,
  hostCards
}: ActivityLibrarySideScrollPlaygroundProps) {
  const [isOpen, setIsOpen] = useState(false);
  const [status, setStatus] = useState(
    "Open the library from Add activity — full-screen drawer over this workshop surface."
  );
  ...
  function openLibrary() {
    setStatus("Activity library side scroll open.");
    setIsOpen(true);
  }
  ...
  return (
    <main className="relative min-h-screen overflow-x-hidden bg-[#F6F1E8] text-[#171614]">
      ...
      <ActivityLibrarySideScroll
        ...
        onClose={() => {
          setStatus("Library closed. Open it again from Add activity.");
          setIsOpen(false);
        }}
      />
    </main>
